...
export default function ConceptInput({ 
  ...
  const handleConceptChange = (inputValue: string) => {
    ...
    if (!isComposing && inputValue) {
      // 한글 자음, 모음, 완성형 한글, 영문, 숫자, 공백, 기본 문장부호 허용
      if (!/^[ㄱ-ㅎㅏ-ㅣ가-힣a-zA-Z0-9\s.,!?]*$/.test(inputValue)) {
        setLocalError('한글, 영문, 숫자, 기본 문장부호만 입력 가능합니다')
        return
      }
    }
  ...
  const handleCompositionEnd = (e: React.CompositionEvent<HTMLInputElement>) => {
    ...
    handleConceptChange(e.currentTarget.value)
  }

  const displayError = error || localError
  const inputId = 'concept-input'
  const errorId = 'concept-error'
  const counterId = 'concept-counter'
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              key={example}
              type="button"
              onClick={() => handleConceptChange(example)}
              disabled={disabled}
